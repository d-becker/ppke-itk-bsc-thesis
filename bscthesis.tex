...
kódbázis karbantartása is sokkal nehezebbé váik, mivel minden változtatást több
...
to overcome these difficulties without undue compromises in performace. We also
...
\subsection{Structured vs. unstructures meshes}
...
connectivity information needs to be stored or looked up.
...
cells are adjacent needs to be stored explicitly, taking up space, and often
necessitating more expensive lookup routines than simple arithmetics.

% TODO: picture of structured and unstructured meshes.
...
hierarchical cache architectures. These consist of multiple layers of memory
storage units organised such that higher-speed storage elements, which are often
also located closer to the processing unit, have lower storage capacity than
...
memory or from a slower cache level is potentially orders of magnitude more
expensive than accessing data in a high-speed cache unit. On many architectures,
...
cache-friendly way may have enormous impact on performace --- indeed,
...
when the processor reads from memory, it can only read a fixed amount of data
even if it is more than immediately required. If the remaining data is the same
as is needed by the next computation cycle, it makes it unnecessary to perform
another (potentially very expensive) load operation from memory.
